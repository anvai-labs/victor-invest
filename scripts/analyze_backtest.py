...
def analyze_backtest(log_file: str = "logs/rl_backtest_sp500_parallel.log"):
    ...
    for sig in ["LONG", "SHORT"]:
        ...
        if sig_rewards:
            ...
            print(
                f"{sig}: {wins}/{len(sig_rewards)} profitable ({wins / len(sig_rewards) * 100:.1f}%), avg: {avg:+.3f}"
            )
    print()
    ...
    print("=" * 70)
    long_sigs = [
        (s["symbol"], s["lookback"], s["gap"], s["long_reward"], s["sector"])
        for s in signals
        if s["signal"] == "LONG" and s["long_reward"] is not None
    ]
    ...
    print("=" * 70)
    short_sigs = [
        (s["symbol"], s["lookback"], s["gap"], s["short_reward"], s["sector"])
        for s in signals
        if s["signal"] == "SHORT" and s["short_reward"] is not None
    ]
