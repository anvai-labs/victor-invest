    ...
    async def run(
        ...
        if not skip_domestic_filter:
            ...
            if foreign_count > 0:
                ...
                logger.info(
                    f"Filtered out {foreign_count} foreign filers (20-F/6-K) - {len(symbols)} domestic filers remaining"
                )
        else:
        ...
        for batch_num in range(total_batches):
            ...
            logger.info(f"\n{'=' * 40}")
            logger.info(f"BATCH {batch_num + 1}/{total_batches} ({batch_start + 1}-{batch_end} of {total_symbols})")
