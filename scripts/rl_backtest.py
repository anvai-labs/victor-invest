...
class RLBacktester:
    ...
    def build_context_features(
        ...
        return ValuationContext(
            ...
            technical_trend=(
                tech_features.price_vs_sma_20 + tech_features.price_vs_sma_50 + tech_features.price_vs_sma_200
            )
            / 3,
    ...
    def get_rl_context_features(self, symbol: str, as_of_date: date) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        """
            ...
            try:
                ...
                if consolidated.insider:
                    ...
                    insider_data = {
                        ...
                        "sentiment_score": (
                            sentiment_score_obj.get("score", 0.0) if isinstance(sentiment_score_obj, dict) else 0.0
                        ),
                        "buy_count": summary.get("buys", 0),
                        "sell_count": summary.get("sells", 0),
                        # buy_value/sell_value from sentiment_score object
                        "buy_value": (
                            sentiment_score_obj.get("buy_value", 0) if isinstance(sentiment_score_obj, dict) else 0
                        ),
                        "sell_value": (
                            sentiment_score_obj.get("sell_value", 0) if isinstance(sentiment_score_obj, dict) else 0
                        ),
        ...
        for months_back in lookback_months:
            ...
            try:
                ...
                if self._current_quarterly_metrics:
                    ...
                    financials["quarters_available"] = fcf_quarters  # Also add for RL context
                else:
                ...
                valuation_gap = (
                    (blended_fv - price_at_prediction) / price_at_prediction if price_at_prediction > 0 else 0
                )
                ...
                exit_date_30d = analysis_date + timedelta(days=30) if multi_period_prices.get("1m") else None
                exit_date_90d = analysis_date + timedelta(days=90) if multi_period_prices.get("3m") else None
                exit_date_180d = analysis_date + timedelta(days=180) if multi_period_prices.get("6m") else None
                exit_date_365d = analysis_date + timedelta(days=365) if multi_period_prices.get("12m") else None
                exit_date_548d = analysis_date + timedelta(days=548) if multi_period_prices.get("18m") else None
                exit_date_730d = analysis_date + timedelta(days=730) if multi_period_prices.get("24m") else None
                exit_date_1095d = analysis_date + timedelta(days=1095) if multi_period_prices.get("36m") else None
