...
class MacroIndicatorCollector(BaseCollector):
    ...
    def collect(self) -> CollectionMetrics:
        ...
        try:
            ...
            self.logger.info(f"Refreshing {len(indicators)} FRED indicators from {len(self.categories)} categories")
            ...
            for indicator in indicators:
                try:
                    ...
                    if last_date:
                        ...
                        self.logger.debug(f"{indicator}: Incremental fetch from {start_date} (last: {last_date})")
                    else:
