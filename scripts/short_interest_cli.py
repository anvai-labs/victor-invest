...
def format_most_shorted_output(data: dict) -> str:
    ...
    lines = []
    lines.append(f"\n{'=' * 75}")
    lines.append("MOST SHORTED STOCKS")
    lines.append(f"{'=' * 75}")
...
Examples:
  ...
  %(prog)s --most-shorted --limit 25    Get 25 most shorted stocks
    ...
    parser.add_argument("symbol", nargs="?", help="Stock ticker symbol")
    ...
    if args.current:
        ...
        result = await tool.execute(action="squeeze", symbol=args.symbol)
        formatter = format_squeeze_output
    elif args.most_shorted:
        result = await tool.execute(action="most_shorted", limit=args.limit)
