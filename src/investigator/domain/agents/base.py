...
class InvestmentAgent(ABC):
    ...
    def _wrap_llm_response(
        ...
        **extra_params,
        ...
        """
        # Extract response payload from Ollama result.
        # Some callers provide {'response': '<json-string>'} while others provide
        # {'response': {...}} directly.
        if isinstance(response, dict):
            # Support both Ollama wrapper payloads ({response: ...}) and raw dict payloads
            # produced by deterministic fallbacks in agents.
        ...
        if format == "json":
            # If already structured JSON, use as-is.
            if isinstance(parsed_response, (dict, list)):
                pass
            elif not parsed_response:
                # Empty/None responses should never be cached as empty strings.
                parsed_response = {}
            else:
                ...
                if extracted_json:
                    ...
                    self.logger.info(
                        "✅ Successfully extracted JSON from LLM response (%s chars)",
                        len(str(extracted_json)),
                    )
                else:
                    # JSON extraction failed - log as DEBUG since retry logic will handle it
                    self.logger.debug(
                        "⚠️  Failed to extract JSON from response (length: %s). Raw response: %s...",
                        len(response_text),
                        response_text[:500],
                    )
                    self.logger.debug(
                        "Full raw response that failed JSON extraction: %s",
                        response_text,
                    )
