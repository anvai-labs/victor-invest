...
class ETFMarketContextAgent(InvestmentAgent):
    ...
    def __init__(self, agent_id: str, ollama_client, event_bus, cache_manager):
        ...
        try:
            ...
            deterministic_config = raw_config.get("valuation", {}).get(
                "deterministic", {}
            )
        ...
        self.sector_etfs = {
            ...
            "energy": SectorMapping(
                ...
                "XLE",
                [],
                "Energy Select Sector SPDR Fund",  # Removed secondary ETFs - primary is sufficient
            ),
    ...
    async def _generate_market_sentiment_analysis(
        ...
        if self.use_deterministic and self.deterministic_market_sentiment_generation:
            ...
            return self._build_fallback_market_sentiment_analysis(
...
{chr(10).join(timeframe_context)}
        ...
        try:
            ...
            if isinstance(market_regime_value, dict):
                ...
                parsed_response["market_regime"] = market_context.get(
    ...
    def _build_fallback_market_sentiment_analysis(
        ...
        short_term_sentiment = timeframe_analysis.get("short_term_21d", {}).get(
            "sentiment"
        )
        if (
            leading_sentiment
            and short_term_sentiment
            and leading_sentiment != short_term_sentiment
        ):
            divergences.append(
        ...
        sector_strength = str(
            (sector_context or {}).get("sector_strength", "neutral") or "neutral"
        )
        medium_relative = (
            (relative_performance or {})
            .get("vs_market", {})
            .get("medium_term", {})
            .get("relative_return", 0.0)
        )
        key_drivers = [
        ...
        return {
            ...
            "cross_timeframe_signals": {
                ...
                "leading_indicators": [
                    f"10d sentiment: {leading_sentiment or 'neutral'}"
                ],
                "primary_conclusion": (
    ...
    def _format_market_data_for_prompt(self, analysis_data: Dict) -> str:
        ...
        lines.append("Market Performance (1M):")
