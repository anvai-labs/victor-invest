...
from investigator.domain.services.toon_formatter import (
    TOONFormatter,
    to_toon_peers,
)
...
class SynthesisAgent(InvestmentAgent):
    ...
    def __init__(
        ...
        self.deterministic_risk_assessment_generation = deterministic_config.get(
            "risk_assessment_generation", True
        )
        self.deterministic_scenario_generation = deterministic_config.get(
            "scenario_generation", True
        )
        self.deterministic_recommendation_generation = deterministic_config.get(
            "recommendation_generation", True
        )
        self.deterministic_action_plan_generation = deterministic_config.get(
            "action_plan_generation", True
        )
        self.deterministic_report_generation = deterministic_config.get(
    ...
    def _parse_llm_response(self, response, default=None):
        ...
        def _parse_json_payload(payload: str, wrapped: bool = False) -> Dict[str, Any]:
            ...
            if repair_malformed_json is not None:
                try:
                    repaired = repair_malformed_json(
                        sanitized or payload, return_objects=True
                    )
        ...
        if isinstance(response, dict):
            ...
            if payload is None and {"response", "model_info", "metadata"} <= set(
                response.keys()
            ):
                return default
            return response
    ...
    def _strip_llm_wrappers(self, value: Any) -> Any:
        ...
        if isinstance(value, dict):
            if "response" in value and (
                "model_info" in value or "metadata" in value or "prompt" in value
            ):
    ...
    def _normalize_scenarios_response(self, payload: Any) -> Dict[str, Any]:
        ...
        if isinstance(nested, dict) and not any(
            payload.get(name) for name in ("bull_case", "base_case", "bear_case")
        ):
            candidate = nested
        ...
        base_case = self._first_non_empty(
            candidate, ["base_case", "base", "neutral_case", "expected_case"]
        )
    ...
    def _normalize_recommendation_response(self, payload: Any) -> Dict[str, Any]:
        ...
        if isinstance(final_recommendation, str) and final_recommendation.strip():
            normalized["final_recommendation"] = (
                final_recommendation.strip().lower().replace(" ", "_")
            )

        if not normalized.get("key_reasons_for_recommendation"):
            reasons = self._first_non_empty(
                normalized, ["key_reasons", "reasons", "rationale"]
            )
        ...
        if not normalized.get("main_risks_to_monitor"):
            risks = self._first_non_empty(
                normalized, ["main_risks", "risks", "risk_factors"]
            )
        ...
        if not normalized.get("exit_conditions"):
            exits = self._first_non_empty(
                normalized, ["exit_strategy", "exits", "stop_conditions"]
            )
    ...
    def _normalize_action_plan_response(self, payload: Any) -> Dict[str, Any]:
        ...
        if isinstance(normalized.get("entry_strategy"), str):
            normalized["entry_strategy"] = {
                "entry_timing_considerations": normalized["entry_strategy"]
            }
    ...
    def _build_deterministic_risk_assessment_payload(
        ...
        for risk in all_risks:
            ...
            categorized.append(
                {"risk": description, "category": category, "priority": priority}
            )
    ...
    def _build_fallback_scenarios_payload(
        ...
        *,
        ...
        current_price = synthesis_input.fundamental_analysis.get("valuation", {}).get(
            "current_price", 100
        )
        ...
        upside_to_fair = (
            ((fair_value - current_price) / current_price) if current_price > 0 else 0.0
        )
        bull_return = 0.25
        base_return = upside_to_fair
        bear_return = -0.20
        expected_return = (
            (0.3 * bull_return) + (0.5 * base_return) + (0.2 * bear_return)
        )
    ...
    def _build_deterministic_recommendation_payload(
        ...
        conviction_level = (
            "high" if confidence >= 75 else "medium" if confidence >= 55 else "low"
        )
        return {
            "final_recommendation": recommendation,
            "conviction_level": conviction_level,
            "position_sizing_suggestion": (
                7 if recommendation in {"strong_buy", "buy"} else 3
            ),
    ...
    def _build_deterministic_action_plan_payload(
        self, synthesis_input: SynthesisInput
    ) -> Dict[str, Any]:
        """Build deterministic action-plan payload."""
        technical_levels = (
            synthesis_input.technical_analysis.get("levels", {})
    ...
    async def process(self, task: AgentTask) -> AgentResult:
        ...
        try:
            ...
            reconciliation_raw = await self._reconcile_conflicts(
                conflicts, synthesis_input
            )
            ...
            action_plan_raw = await self._generate_action_plan(
    ...
    async def _score_analyses(self, synthesis_input: SynthesisInput) -> Dict:
        ...
        if synthesis_input.sec_analysis:
            sec_score = self._evaluate_analysis_quality(
                synthesis_input.sec_analysis,
                required_fields=["metrics", "risks", "analysis"],
            )
        ...
        if synthesis_input.fundamental_analysis:
            fundamental_score = self._evaluate_analysis_quality(
                ...
                required_fields=["valuation", "quality_score", "analysis"],
            )
        ...
        if synthesis_input.technical_analysis:
            technical_score = self._evaluate_analysis_quality(
                ...
                required_fields=["signals", "levels", "analysis"],
            )
        ...
        if synthesis_input.sentiment_analysis:
            sentiment_score = self._evaluate_analysis_quality(
                synthesis_input.sentiment_analysis,
                required_fields=["sentiment_score", "trends"],
            )
    ...
    def _calculate_smart_price_targets(
        ...
        multi_model_summary.get("model_agreement_score")
    ...
    async def _comprehensive_risk_assessment(
        ...
        if self.use_deterministic and self.deterministic_risk_assessment_generation:
            ...
            response_data = self._build_deterministic_risk_assessment_payload(
                all_risks, conflicts
            )
        ...
        if not isinstance(response_dict, dict) or not response_dict:
            ...
            response_dict = self._build_deterministic_risk_assessment_payload(
                all_risks, conflicts
            )
        ...
        if "risk_reward_ratio" not in response_dict:
            ...
            response_dict["risk_reward_ratio"] = round(
                max(0.3, (100.0 - base_risk) / 50.0), 2
            )
    ...
    async def _make_recommendation(
        ...
        if (
            not isinstance(response_dict, dict)
            or not response_dict
            or not response_dict.get("final_recommendation")
        ):
            self.logger.warning(
    ...
    async def _generate_action_plan(
        ...
        if self.use_deterministic and self.deterministic_action_plan_generation:
            ...
            response_data = self._build_deterministic_action_plan_payload(
                synthesis_input
            )
        ...
        if (
            ...
            or not parsed_response
            or not parsed_response.get("entry_strategy")
        ):
            self.logger.warning(
                "Action plan LLM returned empty/invalid payload for %s. Using deterministic fallback.",
                synthesis_input.symbol,
            )
            parsed_response = self._build_deterministic_action_plan_payload(
                synthesis_input
            )
