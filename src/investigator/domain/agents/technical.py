...
class TechnicalAnalysisAgent(InvestmentAgent):
    ...
    def __init__(
        ...
        self.deterministic_signal_generation = deterministic_config.get(
            "technical_signal_generation", True
        )
        self.deterministic_pattern_recognition = deterministic_config.get(
            "technical_pattern_recognition", True
        )
        self.deterministic_technical_report_generation = deterministic_config.get(
    ...
    async def _detect_patterns(
        ...
        if self.use_deterministic and self.deterministic_pattern_recognition:
            self.logger.debug(
                "%s - Using deterministic pattern recognition (LLM bypass)", symbol
            )
    ...
    async def _generate_signals(
        ...
        if self.use_deterministic and self.deterministic_signal_generation:
            self.logger.debug(
                "%s - Using deterministic signal generation (LLM bypass)", symbol
            )
        ...
        if not isinstance(parsed_response, dict) or not parsed_response.get(
            "entry_signal"
        ):
            self.logger.warning(
    ...
    async def _synthesize_technical_report(self, analysis_data: Dict) -> Dict:
        ...
        if self.use_deterministic and self.deterministic_technical_report_generation:
            self.logger.debug(
                "%s - Using deterministic technical synthesis (LLM bypass)", symbol
            )
    ...
    def _build_fallback_patterns(
        ...
        short_return = (
            float((closes.iloc[-1] / closes.iloc[-short_window]) - 1)
            if short_window > 1
            else 0.0
        )
        long_return = (
            float((closes.iloc[-1] / closes.iloc[-long_window]) - 1)
            if long_window > 1
            else short_return
        )
        volatility = float(
            closes.pct_change(fill_method=None).tail(short_window).std() * np.sqrt(252)
        )
        ...
        if not patterns:
            ...
            patterns.append(
                {
                    "pattern_name": "momentum_continuation_bullish"
                    if is_bullish
                    else "momentum_continuation_bearish",
                    "confidence": min(75, int(50 + (abs(short_return) * 300))),
                    "start_index": start_index,
                    "end_index": end_index,
                    "breakout_level": round(current_price, 2),
                    "target_price": round(
                        current_price * (1.06 if is_bullish else 0.94), 2
                    ),
    ...
    def _build_fallback_technical_report(self, analysis_data: Dict) -> Dict[str, Any]:
        ...
        signals = (
            analysis_data.get("signals", {}) if isinstance(analysis_data, dict) else {}
        )
        sentiment = (
        ...
        momentum = (
            analysis_data.get("momentum", {}) if isinstance(analysis_data, dict) else {}
        )
