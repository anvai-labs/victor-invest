...
class DynamicModelWeightingService:
    ...
    def __init__(self, valuation_config: Dict[str, Any]):
        ...
        self.fee_based_insurance_industry_keywords = tuple(
            ...
            for keyword in valuation_config.get(
                "fee_based_insurance_industry_keywords",
                [
                    "insurance broker",
                    "insurance brokers",
                    "insurance agency",
        ...
        self.threshold_registry = ThresholdRegistry(
            sector_thresholds=pe_thresholds.get("sectors", {}),
            industry_overrides=pe_thresholds.get("industries", {}),
        )
    ...
    def determine_weights(
        ...
        # Ensure applicability filters have normalized classification context.
        if sector and not financials.get("sector"):
            financials["sector"] = sector
        if industry and not financials.get("industry"):
        ...
        if ratios.get("dividend_payout_ratio") in (None, 0):
            ratios["dividend_payout_ratio"] = payout_ratio
    ...
    def _resolve_payout_ratio_ratio(self, financials: Dict[str, Any], ratios: Dict[str, Any]) -> float:
        ...
        Preference:
        1. Explicit payout ratio tags (financials/ratios), normalized
        ...
        for value in (
            ...
            ratios.get("payout_ratio"),
            ratios.get("dividend_payout_ratio"),
        ):
        ...
        if payout_ratio >= dividend_threshold:
            ...
            if symbol and symbol.upper() in KNOWN_DIVIDEND_ARISTOCRATS:
                if payout_ratio >= pure_payout_threshold and revenue_growth < pure_growth_cutoff:
                    ...
                    return ("dividend_aristocrat", "dividend_aristocrat_growth")
            elif payout_ratio > max_non_aristocrat_payout:
                logger.info(
                    f"{symbol or 'UNKNOWN'} - Payout ratio {payout_ratio * 100:.1f}% exceeds "
                    f"non-aristocrat threshold {max_non_aristocrat_payout * 100:.0f}%; "
                    "skipping high_dividend_payer tier as potentially unsustainable"
                )
            else:
    ...
    def _apply_company_specific_adjustments(
        ...
        if profitability_stage == "pre_profit" and revenue_growth > 30:
            ...
            if adjusted_weights.get("ps", 0) > 0:
                ...
                adjustments_made.append("Pre-profit high growth → Boosted PS by 50%")
    ...
    def _is_insurance_or_managed_care(
        ...
        if self._is_fee_based_insurance(symbol, industry):
            return False
