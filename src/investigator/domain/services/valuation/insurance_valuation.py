...
def assess_combined_ratio_quality(
    ...
    if combined_ratio < COMBINED_RATIO_THRESHOLDS["excellent"]:
        return (
            "excellent",
            f"Excellent underwriting ({combined_ratio:.1%} vs {target:.0%} target)",
        )
    elif combined_ratio < COMBINED_RATIO_THRESHOLDS["good"]:
        return (
            "good",
            f"Good underwriting ({combined_ratio:.1%} vs {target:.0%} target)",
        )
    elif combined_ratio < COMBINED_RATIO_THRESHOLDS["acceptable"]:
        return (
            "acceptable",
            f"Acceptable underwriting ({combined_ratio:.1%} vs {target:.0%} target)",
        )
...
def value_insurance_company(
    ...
    if (not shares_outstanding or shares_outstanding <= 0) and current_price:
        market_cap = (
            ...
            or financials.get("market_capitalization")
            or financials.get("market_value")
        )
        if market_cap and market_cap > 0:
            try:
                shares_outstanding = float(market_cap) / float(current_price)
                warnings.append(
                    "shares_outstanding derived from market_cap/current_price"
                )
                logger.info(
                    f"{symbol} - Derived shares_outstanding from market cap: {shares_outstanding / 1e6:.1f}M"
                )
            except Exception:
                pass
...
def _determine_target_pb(
    ...
    if net_margin is None:
        ...
        if roe >= 20:
            ...
            confidence = "low"
        logger.info(
            f"{symbol} - ROE-only insurer profile (ROE={roe:.1f}%, Margin=N/A) -> P/BV={target_pb:.2f}x"
        )
        return target_pb, confidence
