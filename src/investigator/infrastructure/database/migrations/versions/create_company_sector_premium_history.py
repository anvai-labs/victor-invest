...
class CompanySectorPremiumHistory(Base):
    ...
    # Sample metadata
    sector_sample_size = Column(Integer, nullable=True)  # Number of companies in sector sample
    ...
    __table_args__ = (
        UniqueConstraint("symbol", "fiscal_year", "fiscal_period", name="uix_company_premium_period"),
    ...
    def __repr__(self):
        return f"<CompanySectorPremiumHistory({self.symbol}, FY:{self.fiscal_year}, PE premium:{self.pe_premium_pct}%)>"
