    ...
    async def check_alerts(self):
        ...
        if health["metrics"]["cpu_percent"] > self.thresholds["cpu_critical"]:
            await self._raise_alert(
                "cpu_critical",
                f"CPU usage critical: {health['metrics']['cpu_percent']:.1f}%",
                "critical",
            )
        elif health["metrics"]["cpu_percent"] > self.thresholds["cpu_warning"]:
            await self._raise_alert(
                "cpu_warning",
                f"CPU usage high: {health['metrics']['cpu_percent']:.1f}%",
                "warning",
            )
        else:
        ...
        if health["metrics"]["memory_percent"] > self.thresholds["memory_critical"]:
            await self._raise_alert(
                "memory_critical",
                f"Memory usage critical: {health['metrics']['memory_percent']:.1f}%",
                "critical",
            )
        elif health["metrics"]["memory_percent"] > self.thresholds["memory_warning"]:
            await self._raise_alert(
                "memory_warning",
                f"Memory usage high: {health['metrics']['memory_percent']:.1f}%",
                "warning",
            )
        else:
