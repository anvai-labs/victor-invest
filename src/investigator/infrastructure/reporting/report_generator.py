...
class PDFReportGenerator:
    ...
    def generate_report(
        ...
        position_size_data = [
            ...
            [
                "LARGE",
                "5-10%",
                "High conviction positions with strong fundamental and technical alignment",
            ],
            [
                "MODERATE",
                "2-5%",
                "Standard positions with good risk-adjusted return potential",
            ],
            [
                "SMALL",
                "0.5-2%",
                "Speculative or higher-risk positions with asymmetric upside",
            ],
            [
                "AVOID",
                "0%",
                "Positions with significant downside risk or poor risk-reward ratio",
            ],
        ]
        ...
        recommendation_data = [
            ...
            [
                "STRONG_BUY",
                "Exceptional opportunity with significant upside potential and strong conviction",
            ],
            [
                "BUY",
                "Attractive investment with positive risk-adjusted return expectations",
            ],
            [
                "HOLD",
                "Fair value with limited upside/downside, suitable for existing positions",
            ],
            [
                "SELL",
                "Overvalued or deteriorating fundamentals warrant position reduction",
            ],
            [
                "STRONG_SELL",
                "Significant downside risk or fundamental concerns warrant avoidance",
            ],
        ]
    ...
    def _create_executive_summary(self, recommendations: list[dict]) -> list:
        ...
        overview_data = [
            ...
            [
                ...
                f"{avg_score:.1f}/10",
                self._get_score_rating(avg_score),
            ],
        ]
            ...
            for i, rec in enumerate(top_recs):
                ...
                card_data = [
                    ...
                    [
                        ...
                        "Upside",
                        (
                            f"{((price_target / current_price - 1) * 100):.1f}%"
    ...
    def _create_symbol_analysis(self, recommendation: dict, include_charts: list[str] | None = None) -> list:
        ...
        if multi_year_trends := recommendation.get("multi_year_trends"):
            if multi_year_metrics := multi_year_trends.get("metrics"):
                ...
                elements.append(
                    Paragraph(
                        f"📈 <b>Multi-Year Historical Trends ({years_analyzed} Years)</b>",
        ...
        if dcf_valuation := recommendation.get("dcf_valuation"):
            ...
            if enterprise_value := dcf_valuation.get("enterprise_value"):
                ...
                elements.append(
                    Paragraph(
                        f"  • Equity Value: ${equity_value:.2f}B",
                        self.styles["AnalysisText"],
                    )
                )
            ...
            if sensitivity := dcf_valuation.get("sensitivity_analysis"):
                ...
                elements.append(
                    Paragraph(
                        "<b>Sensitivity Analysis - Fair Value Table:</b>",
                        self.styles["AnalysisText"],
                    )
                )
                elements.append(
                    Paragraph(
                        "<i>(Varies WACC and Terminal Growth Rate)</i>",
                        self.styles["AnalysisText"],
                    )
                )
                elements.append(Spacer(1, 0.05 * inch))
        ...
        if recession_perf := recommendation.get("recession_performance"):
            ...
            if crisis_performance := recession_perf.get("crisis_performance"):
                elements.append(
                    Paragraph(
                        "<b>Historical Crisis Performance:</b>",
                        self.styles["AnalysisText"],
                    )
                )
                elements.append(Spacer(1, 0.05 * inch))
        ...
        if risk_scores := recommendation.get("risk_scores"):
            ...
            elements.append(
                Paragraph(
                    "⚠️ <b>Multi-Dimensional Risk Assessment</b>",
                    self.styles["Heading3"],
                )
            )
            elements.append(
        ...
        if peer_leaderboard := recommendation.get("peer_leaderboard"):
            if peers := peer_leaderboard.get("peers"):
                ...
                elements.append(
                    Paragraph(
                        f"📊 <b>Peer Performance Leaderboard - {industry}</b>",
                        self.styles["Heading3"],
                    )
                )
        ...
        if monte_carlo := recommendation.get("monte_carlo_results"):
            ...
            elements.append(
                Paragraph(
                    "📊 <b>Monte Carlo Probabilistic Forecast (1 Year)</b>",
                    self.styles["Heading3"],
                )
            )
            elements.append(Spacer(1, 0.1 * inch))
                ...
                for name, scenario in monte_carlo.scenarios.items():
                    ...
                    scenario_data.append(
                        [
                            ...
                            prob,
                            price,
                            Paragraph(return_str, self.styles["Normal"]),
                        ]
                    )

                scenario_table = Table(
                    scenario_data,
                    colWidths=[1.8 * inch, 1.0 * inch, 1.2 * inch, 1.0 * inch],
                )
    ...
    def _create_portfolio_summary(self, recommendations: list[dict]) -> list:
        ...
        summary_table = Table(
            table_data,
            colWidths=[1.2 * inch, 1.8 * inch, 1.2 * inch, 1.3 * inch, 1.5 * inch],
        )
    ...
    def _create_charts_section(self, chart_paths: list[str]) -> list:
        ...
        for chart_path in chart_paths:
            if "2d_technical_fundamental" in chart_path and Path(chart_path).exists():
                elements.append(
                    Paragraph(
                        "<b>Technical vs Fundamental Analysis</b>",
                        self.styles["Heading3"],
                    )
                )
                img = Image(chart_path, width=6 * inch, height=4.5 * inch)
        ...
        for chart_path in chart_paths:
            if "profitability" in chart_path and Path(chart_path).exists():
                elements.append(
                    Paragraph(
                        "<b>Quarterly Profitability Analysis</b>",
                        self.styles["Heading3"],
                    )
                )
                elements.append(
                    Paragraph(
                        "Net income and profit margins by quarter",
                        self.styles["Normal"],
                    )
                )
                img = Image(chart_path, width=6.5 * inch, height=5 * inch)
        ...
        for chart_path in chart_paths:
            if "risk_radar" in chart_path and Path(chart_path).exists():
                elements.append(
                    Paragraph(
                        "<b>Multi-Dimensional Risk Assessment</b>",
                        self.styles["Heading3"],
                    )
                )
                elements.append(
    ...
    def _create_tier2_interpretation_appendix(self) -> list:
        ...
        elements.append(
            Paragraph(
                "Appendix A - Tier 2 Enhancements Interpretation Guide",
        ...
        if technical_score > 0:
            tech_data.append(
                [
                    ...
                    self._get_rating(technical_score),
                ]
            )
    ...
    def _create_entry_exit_section(self, recommendation: dict) -> list:
        ...
        if optimal_entry_zone and current_price > 0:
            ...
            zone_data.append(
                [
                    ...
                    f"{optimal_entry_zone.get('recommended_allocation_pct', 0):.1f}%",
                ]
            )
        ...
        if exit_signals:
            ...
            exit_table = Table(
                exit_data,
                colWidths=[1.5 * inch, 1.2 * inch, 1.2 * inch, 1 * inch, 1 * inch],
            )
            exit_table.setStyle(
                TableStyle(
                    [
