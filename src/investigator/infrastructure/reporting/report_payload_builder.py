...
class ReportPayloadBuilder:
    ...
    def build(
        ...
        payload = {
            ...
            "timestamp": (
                ...
                or (
                    synthesis_report.get("timestamp", "")
    ...
    def _unwrap_response(self, synthesis_report: Any) -> Dict:
        ...
        depth = 0
        while (
            isinstance(data, dict) and "response" in data and depth < max_unwrap_depth
        ):
            ...
            if isinstance(wrapped, str):
                if not wrapped.strip():
                    self.logger.warning(
                        "Response is empty string - returning empty payload"
                    )
                    return {}
                ...
                continue
            self.logger.warning(
                "Response is %s, not dict - returning empty payload", type(wrapped)
            )
            return {}
    ...
    def _extract_scenarios(self, data: Dict) -> Dict:
        ...
        return {
            "bull_case": (
                ...
                or scenarios.get("bull")
                or data.get("bull_case")
            ),
            "base_case": (
                scenarios.get("base_case")
                or scenarios.get("base")
                or data.get("base_case")
            ),
            "bear_case": (
                scenarios.get("bear_case")
                or scenarios.get("bear")
                or data.get("bear_case")
            ),
        }
    ...
    def _extract_action_plan(self, data: Dict) -> Dict:
        ...
        if not action_plan and isinstance(
            data.get("recommendation_and_action_plan"), dict
        ):
        ...
        if not actions and isinstance(action_plan.get("entry_strategy"), dict):
            ...
            actions = (
                [maybe_action] if isinstance(maybe_action, str) and maybe_action else []
            )
