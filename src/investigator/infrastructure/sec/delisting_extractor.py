...
def _symbol_from(submissions: Dict[str, Any], fallback: Optional[str]) -> Optional[str]:
...
async def backfill_delisting(
    ...
    cik: str,
    sec_client: Any,
    last_close_lookup: Optional[Callable[[str], Optional[Tuple[date, float]]]] = None,
) -> Optional[DelistingRecord]:
