...
class TestAgentOrchestrator:
    ...
    def test_analysis_mode_enum(self):
        assert {mode.name for mode in AnalysisMode} >= {
            "QUICK",
            "STANDARD",
            "COMPREHENSIVE",
        }
    ...
    def test_priority_enum(self):
        assert {member.name for member in Priority} >= {
            "CRITICAL",
            "HIGH",
            "NORMAL",
            "LOW",
...
class TestOrchestratorResilience:
    ...
    @pytest.mark.asyncio
    async def test_process_event_reads_task_id_from_event_data(self, caplog):
        orch = AgentOrchestrator(cache_manager=Mock(), metrics_collector=Mock())
