...
from investigator.domain.agents.fundamental.valuation_weighting import (
    resolve_fallback_weights,
)
...
def test_resolve_fallback_weights_uses_dynamic_service_when_available():
    ...
    dynamic_service.determine_weights.return_value = (
        {"dcf": 50.0, "pe": 30.0},
        "tier_2",
        None,
    )
...
def test_resolve_fallback_weights_falls_back_to_static_config():
    ...
    config = SimpleNamespace(
        valuation={"model_fallback": {"growth": {"weights": {"dcf": 0.6, "pe": 0.4}}}}
    )
...
def test_agent_resolve_fallback_weights_delegates_to_helper():
    ...
    agent._resolve_fallback_weights = (
        FundamentalAnalysisAgent._resolve_fallback_weights.__get__(agent)
    )
    ...
    with patch(
        ...
        result = agent._resolve_fallback_weights(
            _profile(), [{"model": "dcf"}], {"book_value": 1}, {"payout_ratio": 1}
        )
