...
def _build_dcf(monkeypatch, quarterly_metrics):
    monkeypatch.setattr(DCFValuation, "_load_dcf_config", lambda self: {"wacc_parameters": {}})
    monkeypatch.setattr(DCFValuation, "_get_company_sector", lambda self: "Consumer Discretionary")
    monkeypatch.setattr(
        DCFValuation,
...
def test_get_shares_outstanding_keeps_normal_values(monkeypatch):
    dcf = _build_dcf(monkeypatch, quarterly_metrics=[{"shares_outstanding": 3_526_000_000}])
