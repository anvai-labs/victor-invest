...
def test_get_validates_only_requested_sector(tmp_path, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="investigator.infrastructure.data.sector_multiples_loader"):
        loader.load()
        loader.get("Technology")

    messages = [record.message for record in caplog.records]
    assert any("Sector multiples for Technology are stale" in message for message in messages)
    assert not any("Sector multiples for Utilities are stale" in message for message in messages)
...
def test_get_stale_recent_logs_info_not_warning(tmp_path, caplog):
    ...
    stale_logs = [record for record in caplog.records if "Sector multiples for Technology are stale" in record.message]
...
def test_get_stale_severe_logs_warning(tmp_path, caplog):
    ...
    stale_warnings = [
        record for record in caplog.records if "Sector multiples for Technology are stale" in record.message
    ]
