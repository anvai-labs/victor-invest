...
def test_handlers_module_uses_compat_layer_imports():
    ...
    assert "from victor_invest.compat.handlers import BaseHandler, handler_decorator" in handlers_py
    assert "from victor.framework.handler_registry import handler_decorator" not in handlers_py
    assert "from victor.framework.workflows.base_handler import BaseHandler" not in handlers_py
...
def test_no_direct_legacy_handler_api_imports_outside_compat_layer():
    ...
    assert not violations, f"Direct legacy handler API imports found: {sorted(set(violations))}"
