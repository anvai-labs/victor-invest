...
class CacheStatsMonitor:
    ...
    def print_cache_report(self, detailed: bool = False):
        ...
        print("-" * 80)
        print(
            f"{'Cache Type':<20} {'Hit Ratio':<12} {'Hits':<8} {'Misses':<8} {'Writes':<8} {'Errors':<8} {'Avg Time':<10}"
        )
        print("-" * 80)
        ...
        for cache_type, type_stats in performance.items():
            ...
            print(
                f"{cache_type:<20} {perf['hit_ratio_pct']:>8.1f}% {ops['hits']:>8} {ops['misses']:>8} {ops['writes']:>8} {ops['errors']:>8} {perf['avg_time_ms']:>8.1f}ms"
            )
            ...
            for cache_type, type_stats in performance.items():
                if type_stats["handlers"]:
                    ...
                    for handler_name, handler_stats in type_stats["handlers"].items():
                        print(
                            f"  • {handler_name:<25} Hit Ratio: {handler_stats['hit_ratio_pct']:>6.1f}% | "
