    ...
    def _analyze_trends(self, quarterly_data: List[QuarterlyData]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(1, len(sorted_data)):
                ...
                # Get financial data directly from the new structure
                current_income = (
                    current.financial_data.income_statement
                    if hasattr(current.financial_data, "income_statement")
                    else {}
                )
                previous_income = (
                    previous.financial_data.income_statement
                    if hasattr(previous.financial_data, "income_statement")
                    else {}
                )
