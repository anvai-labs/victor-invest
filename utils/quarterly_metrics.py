...
class QuarterlyMetricsCalculator:
    ...
    def calculate_all_metrics(
        self, quarterly_data: List[Dict], symbol: str
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            df = df.sort_values(
                ["fiscal_year", "fiscal_quarter"], ascending=[True, True]
            )
            ...
            self.logger.info(
                f"Calculated quarterly metrics for {symbol}: {len(df)} quarters, {len(df.columns)} total columns"
            )
    ...
    def _calculate_growth_metrics(self, df: pd.DataFrame):
        ...
        if "revenue" in df.columns:
            ...
            df["Revenue_YoY_Growth"] = (
                df["revenue"].pct_change(periods=4) * 100
            )  # 4 quarters = 1 year
    ...
    def _calculate_efficiency_ratios(self, df: pd.DataFrame):
        ...
        if "cost_of_revenue" in df.columns and "inventory" in df.columns:
            df["Inventory_Turnover"] = (
                (df["cost_of_revenue"] * 4) / df["inventory"]
            ).fillna(0)
        ...
        if "revenue" in df.columns and "accounts_receivable" in df.columns:
            df["Receivables_Turnover"] = (
                (df["revenue"] * 4) / df["accounts_receivable"]
            ).fillna(0)
    ...
    def _calculate_liquidity_ratios(self, df: pd.DataFrame):
        ...
        if all(
            col in df.columns
            for col in ["current_assets", "inventory", "current_liabilities"]
        ):
            df["Quick_Ratio"] = (
    ...
    def _calculate_leverage_ratios(self, df: pd.DataFrame):
        ...
        if "total_debt" in df.columns and "stockholders_equity" in df.columns:
            df["Debt_to_Equity"] = (
                df["total_debt"] / df["stockholders_equity"]
            ).fillna(0)
        ...
        if "stockholders_equity" in df.columns and "total_assets" in df.columns:
            df["Equity_Ratio"] = (
                df["stockholders_equity"] / df["total_assets"]
            ).fillna(0)
    ...
    def _calculate_valuation_metrics(self, df: pd.DataFrame):
        ...
        if "revenue" in df.columns and "shares_outstanding" in df.columns:
            df["Revenue_Per_Share"] = (df["revenue"] / df["shares_outstanding"]).fillna(
                0
            )
        ...
        if all(
            col in df.columns
            for col in ["current_assets", "current_liabilities", "shares_outstanding"]
        ):
            working_capital = df["current_assets"] - df["current_liabilities"]
            df["Working_Capital_Per_Share"] = (
                working_capital / df["shares_outstanding"]
            ).fillna(0)
    ...
    def _calculate_quality_scores(self, df: pd.DataFrame):
        ...
        if "Revenue_QoQ_Growth" in df.columns:
            df["Revenue_Growth_Consistency"] = (
                df["Revenue_QoQ_Growth"]
        ...
        if "Current_Ratio" in df.columns:
            score_components.append(
                np.clip(df["Current_Ratio"] / 2.0, 0, 1)
        ...
        if "Debt_to_Equity" in df.columns:
            score_components.append(
                np.clip(1 - (df["Debt_to_Equity"] / 2.0), 0, 1)
    ...
    def _assess_data_quality(self, df: pd.DataFrame) -> List[float]:
        ...
        for _, row in df.iterrows():
            available_items = sum(
                ...
                if item in df.columns and pd.notna(row[item]) and row[item] != 0
            )
    ...
    def extract_recent_quarters_for_llm(
        self, enhanced_df: pd.DataFrame, quarters: int = 3
    ) -> pd.DataFrame:
        """
        ...
        sorted_df = enhanced_df.sort_values(
            ["fiscal_year", "fiscal_quarter"], ascending=[True, True]
        )
