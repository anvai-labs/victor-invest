            ...
            if result.returncode == 0:
                ...
                if result.returncode == 0:
                    ...
                    if "apple" in cpu_info or "m1" in cpu_info or "m2" in cpu_info or "m3" in cpu_info:
                        ...
                        logger.info(
                            f"Apple Silicon detected: {total_memory_gb:.1f}GB total, ~{available_for_gpu:.1f}GB available for GPU"
                        )
                ...
                for hw in hardware:
                    ...
                    if any(chip in chip_type for chip in ["m1", "m2", "m3", "apple"]):
                        ...
                        if memory_match:
                            ...
                            logger.info(
                                f"Apple {chip_type.upper()} with {total_gb}GB unified memory, ~{available_gb:.1f}GB for GPU"
                            )
                            return available_gb
