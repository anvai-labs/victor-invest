...
def composite_score(fundamental_score: Optional[float], technical_score: Optional[float]) -> float:
...
def upside_pct(current_price: Optional[float], target: Optional[float]) -> Optional[float]:
    if not current_price or current_price <= 0 or target is None:
        return None
    return round((target / current_price - 1.0) * 100.0, 1)


def derive_rating(composite: float, target_upside_pct: Optional[float]) -> Tuple[str, str]:
...
def derive_price_target(
    blended_fair_value: Optional[float],
    synthesis_target: Optional[float],
