    ...
    def screen_frame(
        ...
        *,
        ...
        for ticker, df in prices_df.groupby("ticker"):
            ...
            if m is None:
                continue
            fv = float(m.get("fair_value_blended")) if pd.notna(m.get("fair_value_blended")) else float("nan")
            ...
            trend_score += (
                1
                if pd.notna(macd.iloc[-1]) and pd.notna(macd_signal.iloc[-1]) and macd.iloc[-1] > macd_signal.iloc[-1]
                else 0
            )
            ...
            agreement = (
                float(m.get("model_agreement_score")) if pd.notna(m.get("model_agreement_score")) else float("nan")
            )
            score = (
