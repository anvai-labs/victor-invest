    ...
    def _derive_missing_q4_quarters(self, quarters_data: list, symbol: str) -> list:
        ...
        for fy in sorted(fy_groups.keys(), reverse=True):
            ...
            if has_fy and has_q1 and has_q2 and has_q3 and not has_q4:
                fy_entry = next(
                    q for q in fy_quarters if q.get("fiscal_period") == "FY"
                )
                q1_entry = next(
                    q for q in fy_quarters if q.get("fiscal_period") == "Q1"
                )
                q2_entry = next(
                    q for q in fy_quarters if q.get("fiscal_period") == "Q2"
                )
                q3_entry = next(
                    q for q in fy_quarters if q.get("fiscal_period") == "Q3"
                )

                # Helper to safely subtract values (handle None/missing)
                def subtract_metric(fy_val, q1_val, q2_val, q3_val):
                    vals = [
                        v for v in [fy_val, q1_val, q2_val, q3_val] if v is not None
                    ]
                    if len(vals) == 4:
                ...
                derived_q4["income_statement"] = {
                    ...
                    "depreciation_amortization": derived_q4[
                ...
                # Add all Q1, Q2, Q3 quarters and the derived Q4
                # Sort by period_end_date to ensure correct ordering
                fy_quarterly_periods = [
                    q
                    for q in fy_quarters
                    if q.get("fiscal_period") in ["Q1", "Q2", "Q3"]
                ]
                fy_quarterly_periods.append(derived_q4)

                # Sort by period_end_date (descending = most recent first)
                fy_quarterly_periods.sort(
                    key=lambda x: x.get("period_end", ""), reverse=True
                )

                derived_quarters.extend(fy_quarterly_periods)

                q4_derived_count += 1
                # Safe logging that handles None values
                ni_val = derived_q4.get("net_income")
                rev_val = derived_q4.get("total_revenue")
                ni_str = f"${ni_val / 1e6:.0f}M" if ni_val is not None else "N/A"
                rev_str = f"${rev_val / 1e9:.2f}B" if rev_val is not None else "N/A"
                logger.info(
